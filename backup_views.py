    ...
    def post(self, request):
        try:
            ...
            if 'creation_date' in body['stockdata']:
                stock.create_date = datetime.datetime.strptime(str(body['stockdata']['creation_date']),
                                                               "%d/%m/%Y").date()
...
class CustomersReport(APIView):
    def post(self, request):
        try:
            ...
            file_path = BASE_DIR + "/Admin/report_files/customer_debits.csv"
            with open(file_path, 'wb+') as csvfile:
                ...
                for temp in main_data:
                    ...
                    if str(temp['fields']['customer']) in customer_details:
                        customer_details[str(temp['fields']['customer'])]['total_price'] += temp['fields'][
                            'total_price']
                        customer_details[str(temp['fields']['customer'])]['total_quantity'] += temp['fields'][
                            'total_quantity']
                    ...
                    # initialize fields
                        ...
                        customer_details[str(temp['fields']['customer'])]['total_quantity'] = temp['fields'][
                            'total_quantity']
                        customer_details[str(temp['fields']['customer'])]['due'] = \
                        customer_details[str(temp['fields']['customer'])]['due']
                        customer_details[str(temp['fields']['customer'])]['total_paid'] = \
                        customer_details[str(temp['fields']['customer'])]['total_paid']
            ...
            file_path = BASE_DIR + "/Admin/report_files/customer_dues_sum.csv"
            with open(file_path, 'wb+') as csvfile:
                spamwriter = csv.writer(csvfile)
...
class HarvestersReport(APIView):
    ...
    def post(self, request):
        try:
            ...
            # bill date must be between start and end date
            filter_args = {
                '{0}__{1}'.format('created_date', 'gte'): datetime.datetime(int(divided_start_date[2]),
                                                                            int(divided_start_date[1]),
                                                                            int(divided_start_date[0])),
                '{0}__{1}'.format('created_date', 'lte'): datetime.datetime(int(divided_end_date[2]),
                                                                            int(divided_end_date[1]),
                                                                            int(divided_end_date[0]), 23, 59, 59),
            ...
            file_path = BASE_DIR + "/Admin/report_files/harvester_debits.csv"
            with open(file_path, 'w') as csvfile:
                spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
                spamwriter.writerow(
                ...
                for temp in main_data:
                    ...
                    if str(temp['fields']['harvester']) in harvester_details:
                        harvester_details[str(temp['fields']['harvester'])]['advance'] += temp['fields'][
                            'harvester_advance']
                        harvester_details[str(temp['fields']['harvester'])]['total_payment'] += temp['fields'][
                            'harvester_payment']
                    # initialize fields
                    else:
                        ...
                        harvester_details[str(temp['fields']['harvester'])]['advance'] = temp['fields'][
                            'harvester_advance']
                        harvester_details[str(temp['fields']['harvester'])]['due'] = float(
                            harvester_details[str(temp['fields']['harvester'])]['total_payment']) - float(
                            harvester_details[str(temp['fields']['harvester'])]['advance'])
            ...
            all_harvesters = []
            all_borrowers = []
            file_path = BASE_DIR + "/Admin/report_files/harvester_credits.csv"
            with open(file_path, 'w') as csvfile:
                spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            ...
            file_path = BASE_DIR + "/Admin/report_files/harvester_dues_sum.csv"
            with open(file_path, 'w') as csvfile:
                spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    ...
    def post(self, request):
        try:
            ...
            with open(file_path, 'w') as csvfile:
                spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
                spamwriter.writerow(
                    ["farmer Name", "rate_per_ton_farmer", "rate_per_ton_harvester", "total_quantity", "total_quality",
                     "payment", "advance", "due"])
                ...
                for temp in main_data:
                    ...
                    if str(temp['fields']['farmer']) in farmer_details:
                        ...
                        if temp['fields']['farmer_payment']:
                            farmer_details[str(temp['fields']['farmer'])]['total_payment'] += temp['fields'][
                                'farmer_payment']
                        farmer_details[str(temp['fields']['farmer'])]['due'] = float(
                            farmer_details[str(temp['fields']['farmer'])]['total_payment']) - float(
                            farmer_details[str(temp['fields']['farmer'])]['advance'])
                    # initialize fields
                    else:
    ...
    def post(self, request):
        try:
            ...
            if len(stocklist):
                ...
                for temp in main_data:
                    if (len(temp['fields']['products_list'])):
                        ...
                        for tt in products_data_temp:
                            ...
                            tt['fields']['specific_product_data'] = json.loads(
                                serializers.serialize('json', temp_product))
                            tt['fields']['specific_product_data'][0]['fields']['item_name'] = temp_product[
                                0].item_name.name
                            if tt['fields']['specific_product_data'][0]['fields']['item_name'] in types_data:
                                types_data[tt['fields']['specific_product_data'][0]['fields']['item_name']]['price'] += \
                                tt['fields']['price']
                ...
                customer_aggregation = Billing.objects.filter(**filter_args).values('customer').annotate(
                ...
                with open(file_path, 'w') as csvfile:
                    ...
                    for key in types_data.keys():
                        ...
                        spamwriter.writerow(
                            [temp_dict['customer_name'], temp_dict['bill_date'], key, types_data[key]['price'],
                             types_data[key]['quantity']])
                        result_products_list.append(temp_dict)
                return Response({"stocks_list": result_products_list, 'customer_details': customer_aggregation,
                                 'types_data': types_data}, status=200)
...
class BillManagement(View):
    def get(self, request):
        try:
            ...
            if 'billId' in request.GET:
                ...
                return json_response({"bill_info": bill_info, 'product_list': product_list, 'customers': customers},
                                     status=200)
    ...
    def post(self, request):
        ...
        try:
            ...
            billingObject.save()
            billingObject.products_list.fdd(*listOfProducts)
            billingObject.save()
    ...
    def post(self, request):
        ...
        try:
            ...
            if 'created_date' in body['stockdata']:
                print 'creation data'
                stock.create_date = datetime.datetime.strptime(str(body['stockdata']['created_date']),
                                                               "%d/%m/%Y").date()
